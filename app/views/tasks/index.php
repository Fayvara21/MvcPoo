...
<?php foreach ($tasks as $task): ?>
    ...
    <!-- MAIN TASK ROW -->
    <tr class="task-row border-top" style="cursor:pointer;" data-task="<?= $taskId ?>">
        ...
        <td>
            ...
            <div class="small text-muted task-description" style="display:none;"><?= e($task['description']) ?></div>
        </td>
    ...
    <?php foreach ($approList as $a): ?>
        <?php $subIndex++; ?>
        <tr class="task-sub task-<?= $taskId ?> bg-light" style="display:none;">
            <td><?= $subIndex ?></td>
            <td colspan="6">
...
document.querySelectorAll('.task-row').forEach(row => {
    row.addEventListener('click', () => {
        ...
        // Toggle sub-tasks
        document.querySelectorAll('.task-' + taskId).forEach(subRow => {
